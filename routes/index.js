...
router.get("/fetchSavedCourses", async (req, res) => {
    try {
        ...
        const savedCourses = await Course.find().populate('department').populate('professors');
        const departments = await Department.find({}, 'name'); // Fetch only the department names
        savedCourses.forEach(course => {
            course.professors = course.professors.map(professor => professor.name);
        });
        res.json({ savedCourses, departments, allData });
    } catch (error) {
        console.error("Error fetching saved courses:", error);
        res.status(500).json({ error: "Error fetching saved courses" });
    }
});
